package kr.co.ooweat.springScheduler.service;

import kr.co.ooweat.springScheduler.common.Alert;
import kr.co.ooweat.springScheduler.common.Util;
import kr.co.ooweat.springScheduler.mappers.maria.manager.ManagerMapper;
import kr.co.ooweat.springScheduler.model.AlertModel;
import kr.co.ooweat.springScheduler.model.ServiceList;
...
public class BatchService {
    ...
    public void ooweatMariaNodeCountCheck() throws UnsupportedEncodingException, MessagingException {
        ...
        if (galleraNodeCount < 3) {        //현재 node 의 갯수는 3개이다.
            Alert.kakaoAlert(alertModelMaking("system", "RM", "MariaNode", "※DATABASE", "Maria 노드점검요망(정상: 3) / 노드 Count: " + galleraNodeCount, Util.getCurrentDateTime()));
            Alert.smtp(alertModelMaking("system", "RM", "MariaNode", "※DATABASE", "Maria 노드점검요망(정상: 3) / 노드 Count: " + galleraNodeCount, Util.getCurrentDateTime()));
        } else {
            log.info("MariaDB Node Count::{}", galleraNodeCount + " //정상");
        }
    }

    //서비스 프로세스 생존여부 확인 메소드
    public void processAliveCheck() throws MessagingException, UnsupportedEncodingException {
        try {
            ...
            Alert.smtp(alertModelMaking(
                    ...
                    "KakaoAlertProcess",
                    "프로세스 확인",
                    ServiceList.SERVICE_KAKAO_ALERT.getDescription(), Util.getCurrentDateTime()));
